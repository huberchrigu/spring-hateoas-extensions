 ...
 * @author Christoph Huber
 */
infix fun <R : RepresentationModel<R>> Mono<R>.add(links: (R) -> Publisher<Link>): Mono<R> = flatMap { model ->
...
open class WebFluxLinkBuilderDsl<C, R : RepresentationModel<R>>(
     ...
     */
    fun <R : Any> linkTo(func: C.() -> R): WebFluxLinkBuilder.WebFluxBuilder = linkTo(methodOn(controller).func())
