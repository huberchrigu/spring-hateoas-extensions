...
class WebFluxLinkBuilderDslUnitTest {
    ...
    @Test
    fun `should add templated parameters`() {
        val link = linkTo<CustomerController> { findById(null, null) } withRel SELF
        StepVerifier.create(link)
            .expectNextMatches {
                assertThat(it.isTemplated).isTrue()
                assertThat(it.href).isEqualTo("/customers/{id}{?excerpt}")
                true
            }
            .verifyComplete()
    }
    ...
    interface CustomerController {
        ...
        @GetMapping("/{id}")
        fun findById(@PathVariable id: String?, @RequestParam excerpt: Boolean?): Mono<ResponseEntity<CustomerModel>>
